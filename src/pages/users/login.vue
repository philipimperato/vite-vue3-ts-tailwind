...
div(class="min-h-screen bg-gray-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8")
  div(class="sm:mx-auto sm:w-full sm:max-w-md")
    ...
    h2.mt-6.text-center.text-3xl.font-extrabold.text-gray-900 Sign in to your account
      ...
      form.space-y-6(action="#" method="POST")
        div
          label.block.text-sm.font-medium.text-gray-700(for="email")
            | Email address
          .mt-1
            input#email(class="appearance-none block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" name="email" type="email" autocomplete="email" required)
        div
          label.block.text-sm.font-medium.text-gray-700(for="password")
            | Password
          .mt-1
            input#password(class="appearance-none block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" name="password" type="password" autocomplete="current-password" required)
        .flex.items-center.justify-between
          .flex.items-center
            input#remember-me(class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded" name="remember-me" type="checkbox")
            label.ml-2.block.text-sm.text-gray-900(for="remember-me")
              | Remember me
          .text-sm
            a(class="font-medium text-indigo-600 hover:text-indigo-500" href="#")
        ...
        .mt-6.grid.grid-cols-3.gap-3
          div
            a(class="w-full inline-flex justify-center py-2 px-4 border border-gray-300 rounded-md shadow-sm bg-white text-sm font-medium text-gray-500 hover:bg-gray-50" href="#")
              span.sr-only Sign in with Facebook
